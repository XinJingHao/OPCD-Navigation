...
# Env related:
parser.add_argument('--targetpoint', type=tuple, default=(-9, 9), help='target point')
...
if __name__ == '__main__':
    ...
    # 环境初始化
    env_info = env.reset(start=(0,0),target=opt.targetpoint) # (x,y)
    ...
    while not rospy.is_shutdown():
        ...
        pre_robot_loc = np.array(env_info['start_point'])
        '''刷新环境信息'''
        env_info = env.get_envInfo()
        crt_robot_loc = np.array(env_info['start_point'])
        if np.linalg.norm(crt_robot_loc - pre_robot_loc) > 1:
            # 如果机器人距离发生突变，说明机器人被ColorDynamic给reset了，此时OkayPlan也要跟着Priori_Path_Init
            planner.Priori_Path_Init(env_info['start_point'], env_info['target_point'])
